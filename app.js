...
App = {
  init: function() {
    var $body, $document, $window, animating;
    animating = false;
    $window = $(window);
    $body = $('body');
    $document = $(document);
    return $(function() {
      var animate, list_sections, sections;
      sections = [];
      list_sections = function() {
        return sections = _.map($('.c-mosaic-item'), function(section) {
          var jquery;
          jquery = $(section);
          return {
            dom: section,
      ...
      list_sections();
      animate = function(event, elem) {
        var animation, current_height, future_document_height, future_height, scroll, scrollTop, window_height;
        if (animating) {
          return;
        }
        animating = true;
        ...
        animation = _.map(sections, function(section) {
          var offset;
            ...
            complete: function() {
              return setTimeout(function() {
                ...
                _.each(sections, function(section) {
                  return section.jquery.removeAttr("style");
                });
                list_sections();
        ...
        return $.Velocity.RunSequence(animation);
      };
      $body.on('click', '.c-close', function(event) {
        return animate(event, null);
      });
      return $body.on('click', '.c-mosaic-item', function(event) {
        if ($body.hasClass('summary')) {
          return animate(event, this);
        }
      });
    });
  }
